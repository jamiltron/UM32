...
insertR regs i val = let (x,y) = splitAt i regs
                     in (x ++ [val] ++ drop 1 y)
...
allocate b c = let (b',c') = fi2 b c
               in do
                 (UM32 stat regs arrs) <- get
                 let arrID = minID arrs
                 let regs' = insertR regs b' arrID
                 let arrs' = newA arrs arrID c'
                 put (UM32 stat regs' arrs')
--  return ()
               ...
               in do
                 ...
                 case arrayByID arrs (regs !! b') of
                   ...
                   Just arr -> let arrs' = replaceA arrs 0 arr
                               in unless (isNothing arrs') $
                                  put (UM32 (Running (regs !! c')) regs (fromJust arrs'))
                 return ()
...
code = do
  ...
  case s of
    Halted    -> return ()
    Running i -> case arrayByID a 0 of
      Nothing  -> return ()
      Just arr -> do
